...
export const gates = {
  ...
  'access-admin': (user: UserModel | null) => {
    return user !== null && ['owner', 'admin'].includes(String((user as any).team_role || ''))
  },
  ...
  'send-campaigns': (user: UserModel | null) => {
    return user !== null && ['owner', 'admin', 'editor'].includes(String((user as any).team_role || ''))
  },

  'manage-billing': (user: UserModel | null) => {
    return user !== null && String((user as any).team_role || '') === 'owner'
  },
