 ...
 * into an HttpOnly cookie. commshq's `.stx` pages are server-rendered
 * directly from SQLite (no client hydration, no localStorage token), so the
 * server resolves "who's logged in" from this cookie rather than from any
 * Authorization header the browser cannot supply on a document navigation.
 *
