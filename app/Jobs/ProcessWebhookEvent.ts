  ...
  async handle(input: ProcessWebhookPayload) {
    ...
    try {
      ...
      if (event.provider === 'twilio' && !payload.MessageStatus) {
        ...
        if (intent === 'opt-out') {
          await ensureSuppressed({ teamId: input.teamId, channel: 'sms', recipient, source: `twilio:${keyword}` })
          await appendConsentOnce({ teamId: input.teamId, recipient, channel: 'sms', action: 'revoked', source: 'twilio_inbound', idempotencyKey, proof: JSON.stringify({ eventId: event.providerEventId, keyword }) })
        }
          ...
          await appendConsentOnce({ teamId: input.teamId, recipient, channel: 'sms', action: 'granted', source: 'twilio_inbound', idempotencyKey, proof: JSON.stringify({ eventId: event.providerEventId, keyword }) })
        }
      }
      ...
      if (event.provider === 'mail' && ['bounce', 'complaint'].some(kind => String(event.type).includes(kind))) {
        await ensureSuppressed({
          ...
          channel: 'email',
          recipient,
          reason: String(event.type).includes('complaint') ? 'complaint' : 'bounce',
          source: 'owned_mail',
        })
      }
