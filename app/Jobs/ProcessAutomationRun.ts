...
export default new Job({
  ...
  async handle(payload: AutomationPayload) {
    ...
    if (!run || isTerminalAutomationStatus(run.status)) return { skipped: true }
    ...
    const nodes = automationNodes(version.graph)
    ...
    for (const node of nodes) {
      ...
      const step = existing ?? await AutomationStepRun.forceCreate({ team_id: payload.teamId, automation_run_id: run.id, nodeId: String(node.id), type: String(node.type), status: 'running', attempt: 1, idempotencyKey: automationStepKey(run.id, version.version, node.id), input: JSON.stringify(node.config || {}), startedAt: new Date().toISOString() })
      ...
      if (node.type === 'delay') {
        const seconds = automationDelaySeconds(node.config?.seconds)
        await AutomationStepRun.forceUpdate(step.id, { status: 'waiting' })
        await run.update({ status: 'waiting', currentNodeId: String(node.id) })
